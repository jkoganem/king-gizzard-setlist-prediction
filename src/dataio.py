...
# =============================================================================
# The following functions are not currently used in active code but kept for
# potential use during experiments. Will be removed before final commit if unused.
# See TODO.md for details.
# =============================================================================


def load_config_json(config_name: str) -> Dict[str, Any]:
    """Load a configuration JSON file from configs directory.

    Args:
        config_name: Name of config file (e.g., 'aliases.json').
    ...
    """
    config_path = settings.paths.configs_dir / config_name
    return load_json(config_path)


def load_raw_setlists(file_path: Path) -> List[Dict[str, Any]]:
    """Load raw setlist data from JSON file.

    Args:
        file_path: Path to raw setlists JSON file.

    Returns:
        List of raw setlist dictionaries.
...
def save_raw_setlists(setlists: List[Dict[str, Any]], file_path: Path) -> None:
    """Save raw setlist data to JSON file.

    Args:
        setlists: List of setlist dictionaries.
        file_path: Output file path.
...
class ModelArtifacts:
    ...
    Attributes:
        ...
        info_path: Path to model metadata JSON.
    ...
    def exists(self) -> bool:
        """Check if both model and info files exist.

        Returns:
            True if both files exist, False otherwise.

        """
        return self.model_path.exists() and self.info_path.exists()
    ...
    def save(self, model: Any, info: Dict[str, Any]) -> None:
        """Save model and info.

        Args:
            model: Model object to save.
            info: Model metadata dictionary.

        """
        save_model(model, self.model_path)
        save_json(info, self.info_path)


def get_set_model_artifacts() -> ModelArtifacts:
    """Get paths for set prediction model artifacts.

    Returns:
        ModelArtifacts instance for set model.

    """
    base = settings.paths.artifacts_dir / "set_model"
...
def get_opener_closer_artifacts() -> Tuple[ModelArtifacts, ModelArtifacts]:
    """Get paths for opener and closer model artifacts.

    Returns:
        Tuple of (opener_artifacts, closer_artifacts).

    """
    base = settings.paths.artifacts_dir / "oc"
    opener = ModelArtifacts(
        model_path=base / "opener_model.pkl",
        info_path=base / "model_info.json",
    )
    closer = ModelArtifacts(
        model_path=base / "closer_model.pkl",
        info_path=base / "model_info.json",
    )
